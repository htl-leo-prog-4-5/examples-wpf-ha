...
public class WindowNavigator : BaseWindowNavigator, IWindowNavigator
{
    ...
    public async Task ShowBacklogCommentWindowAsync(int backlogItemId)
    {
        ...
        {
            var window = scope.ServiceProvider.GetRequiredService<BacklogCommentWindow>();
            var vm     = (BacklogCommentViewModel)window.DataContext;
            vm.BacklogItemId = backlogItemId;
            window.ShowDialog();
