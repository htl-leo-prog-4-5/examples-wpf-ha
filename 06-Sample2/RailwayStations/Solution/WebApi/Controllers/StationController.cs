...
public class StationController : ControllerBase
{
    ...
    /// <summary>
    /// Get all StationOverview.
    /// </summary>
    /// <returns></returns>
    [HttpGet("overview")]
    public async Task<ActionResult<IEnumerable<StationOverview>>> GetOverviewAsync()
    {
        var allEntities = await _uow.StationRepository.GetStationOverviewAsync();
    ...
    /// <returns></returns>
    [HttpGet]
    public async Task<ActionResult<IEnumerable<StationDto>>> GetAsync(string? sort)
    {
        Func<IQueryable<Station>, IOrderedQueryable<Station>>? orderBy =
            sort switch
            {
                nameof(Station.Id)   => (query) => query.OrderBy(o => o.Id),
                nameof(Station.Name) => (query) => query.OrderBy(o => o.Name),
                _                    => null
            };

        var allEntities = await _uow.StationRepository.GetNoTrackingAsync(
            null,
            orderBy,
            nameof(Station.City),
    ...
    public async Task<ActionResult<StationDto>> AddAsync([FromBody] StationDto value)
    {
        using (var trans = _uow.BeginTransaction())
        {
            ...
            return Created(newUri, ToDto(await _uow.StationRepository.GetByIdAsync(newId,
                nameof(Station.City),
                nameof(Station.RailwayCompanies),
                nameof(Station.Infrastructures),
                nameof(Station.Lines)
