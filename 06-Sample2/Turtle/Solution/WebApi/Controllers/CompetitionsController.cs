...
public class CompetitionsController : ControllerBase
{
    ...
    public record CompetitionVoteDto(
        int ScriptId,
        int Rate);
    ...
    /// <summary>
    /// Add votes to a competition.
    /// </summary>
    /// <param name="id"></param>
    /// <param name="value"></param>
    /// <returns></returns>
    [HttpPost("{id:int}/vote")]
    public async Task<ActionResult<CompetitionDto>> AddCompetitionVotesAsync(int id, [FromBody] IList<CompetitionVoteDto> value)
    {
        using (var trans = _uow.BeginTransaction())
        {
            var entity = await _uow.CompetitionRepository.GetByIdAsync(id);

            var scriptIds = value.Select(v => v.ScriptId);
            var scripts   = (await _uow.ScriptRepository.GetAsync(script => scriptIds.Contains(script.Id))).ToDictionary(s => s.Id, s => s);

            if (entity is null || scripts.Count != scriptIds.Count())
            {
                return NotFound();
            }
            ...
            var votes = value.Select(s => new Vote()
            {
                Competition = entity,
                Script      = scripts[s.ScriptId],
                Created     = DateTime.Now,
                Rating      = s.Rate
            }).ToList();


            await _uow.VoteRepository.AddRangeAsync(votes);
    ...
    /// <returns></returns>
    [HttpGet("{id:int}/voteresult")]
    public async Task<ActionResult<CompetitionVoteResult>> GetVoteResultAsync(int id)
    {
        var result = await _uow.CompetitionRepository.GetCompetitionResult(id);
        return await this.NotFoundOrOk(result);
    }
    ...
    public async Task<ActionResult<CompetitionDto>> AddAsync([FromBody] CompetitionDto value)
    {
        using (var trans = _uow.BeginTransaction())
        {
            ...
            var scriptIds = entity.Scripts!.Select(s => s.Id).ToList();

            entity.Scripts = await _uow.ScriptRepository.GetAsync(script => scriptIds.Contains(script.Id));
