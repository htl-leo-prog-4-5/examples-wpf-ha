...
public class RegistrationConfigViewModel : BaseViewModel
{
    ...
    public RelayCommand UpdateReasons => new RelayCommand(async _ => await SaveReasonsAsync(), _ => _dbReasons != ReasonsText);
    public RelayCommand UpdateTypes   => new RelayCommand(async _ => await SaveTypesAsync(),   _ => _dbTypes != TypesText);
    public RelayCommand CloseWindow   => new RelayCommand(_ => Controller!.CloseWindow());
