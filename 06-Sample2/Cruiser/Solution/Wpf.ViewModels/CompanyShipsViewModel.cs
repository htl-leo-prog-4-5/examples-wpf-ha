...
public class CompanyShipsViewModel : ValidatableBaseViewModel
{
    ...
    public CruiseShip? SelectedShip
    {
        ...
        set
        {
            if (SetProperty(ref _selectedShip, value))
            {
                ...
                {
                    Passengers = _selectedShip.Passengers;
                    Crew       = _selectedShip.Crew;
                }
            }
        }
    }
    ...
    public RelayCommand EditShipCommand   => new RelayCommand(EditMove,                       () => SelectedShip != null && IsNotEditMode);
    public RelayCommand UpdateShipCommand => new RelayCommand(async () => await UpdateShip(), () => IsEditMode);
    ...
    {
        var shipInDb = (await _uow.CruiseShipRepository.GetByIdAsync(SelectedShip!.Id)) ?? throw new ArgumentNullException();
        shipInDb.Passengers = Passengers;
        shipInDb.Crew       = Crew;
    ...
    {
        var ships = await uow.CruiseShipRepository.GetByCompanyIdNoTrackingAsync(CompanyId);
