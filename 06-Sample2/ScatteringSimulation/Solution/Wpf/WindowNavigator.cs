...
public class WindowNavigator : BaseWindowNavigator, IWindowNavigator
{
    ...
    public async Task ShowBacklogCommentWindowAsync(int backlogItemId)
    {
        ...
        {
            var window = scope.ServiceProvider.GetRequiredService<SimulationPreviewWindow>();
            var vm     = (SimulationPreviewViewModel)window.DataContext;
            vm.SimulationId = backlogItemId;
            window.ShowDialog();
