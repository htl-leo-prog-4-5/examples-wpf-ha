...
namespace Survey.Logic
{
	public class SurveyController
	{
		...
		public async Task<bool> Answer(int pollid, int answerid, string username)
		{
			...
			{
// LF3: 6.)
                if (poll.PollEndTime.HasValue && poll.PollEndTime.Value < DateTime.Now)
                    throw new ArgumentException("Timer over");
// --------
				...
				{
// LF3: 4.)
					rep.AddVote(new Repository.Entities.PollVote() { Poll = poll, PollAnswerID = answerid, UserName = username, VoteTime = DateTime.Now });
// --------
...
// LF3: 3.)
            pollentity.PollAnswers = pollentity.PollAnswers.Where((a) => !string.IsNullOrEmpty(a.Answer)).ToArray();
// --------
