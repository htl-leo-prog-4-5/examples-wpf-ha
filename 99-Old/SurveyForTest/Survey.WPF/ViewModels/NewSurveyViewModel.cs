...
// LF3: 2.)
        public Action InvalidSurveyMessageBox { get; set; }
// -------

        Models.NewSurvey _newSurvey = new Models.NewSurvey() { PollEndTime = DateTime.Now + new TimeSpan(1, 0, 0, 0) };
...
// LF3: 2.)
                if (string.IsNullOrEmpty(_newSurvey.Name))
                {
                    InvalidSurveyMessageBox?.Invoke();
                }
                else
                {
// -------
                    ...
                    DTO.Poll poll = new DTO.Poll()
                    {
                        ...
                        PollEndTime = _newSurvey.PollEndTime,
